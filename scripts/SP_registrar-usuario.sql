...
BEGIN
    ...
    /* El procedimiento primero verifica que haya un rol con el in_rol_id dado de parametro*/
    IF EXISTS(SELECT * FROM roles WHERE in_rol_id = rol_id) THEN
		INSERT INTO users (rol_id, username, password, firstname, surname, email, state) 
		VALUES (in_rol_id, in_username, in_password, in_firstname, in_surname, in_email, @DEFAULT_STATE);
